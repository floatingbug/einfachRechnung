...
const taxTotal = computed(() => {
	return Object.values(taxBreakdown.value).reduce((a, b) => a + b, 0);
});

const grossTotal = computed(() => {
	return netTotal.value + taxTotal.value;
});
</script>
	...
	<div class="create-invoice">
		...
		<form class="invoice-form">
			...
			<fieldset>
				...
				<div class="totals">
					<div>Netto: {{ netTotal.toFixed(2) }} €</div>

					<div v-for="(value, rate) in taxBreakdown" :key="rate">
						Steuer ({{ rate }}%): {{ value.toFixed(2) }} €
					</div>

					<div>Steuer gesamt: {{ taxTotal.toFixed(2) }} €</div>
					<div class="gross">Brutto: {{ grossTotal.toFixed(2) }} €</div>
				</div>
			</fieldset>
